 ...
 */
public class BlockChanceLevel extends SequencedGameSlice {
    /**
     ...
     */
    public BlockChanceLevel(final SequencedGameSlice before, final World world) {
    ...
    private void createElems(final World world) {
        ...
        if (RandomValue.get().nextDouble() > 0.5) {
            ...
            getElems().add(new ShootingObstacle(world,
                    new Vector2((float) ((this.getStartPoint().x) / 50 + range * RandomValue.get().nextDouble()), 1.2f)));
        }
    }
}
