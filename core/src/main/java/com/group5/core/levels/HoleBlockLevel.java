 ...
 */
public class HoleBlockLevel extends GameSlice {
    /**
     * constructor.
     *
     ...
     * @author Levi
     */
    public HoleBlockLevel(final World world) {
     ...
     * @param world  physicsworld
     */
    public HoleBlockLevel(final GameSlice before, final World world) {
    ...
    private void createElems(final World world) {
        ...
        if (RandomValue.get().nextDouble() > 0.5) {
            getElems().add(new BlockObstacle(world,
                    new Vector2((float) ((this.getStartPoint().x) / 50 + range * RandomValue.get().nextDouble()), 1.2f)));
        } else {
            getElems().add(new ShootingObstacle(
                    world, new Vector2((float) ((this.getStartPoint().x) / 50 + (range + 1025) * RandomValue.get().nextDouble()), 1.2f)));
        }
    }
}
